...
; CONFIG1
  CONFIG FEXTOSC = OFF ; External Oscillator Selection (EC (external clock) above 8 MHz)
...
; CONFIG3
  ...
  CONFIG PWRTS = PWRT_16 ; Power-up timer selection bits (PWRT is disabled)
  ...
  CONFIG BOREN = OFF ; Brown-out Reset Enable bits (Brown-out Reset enabled , ((BORCON) and 0FFh), 7, b bit is ignored)
...
; CONFIG5
  ...
  CONFIG WDTE = OFF ; WDT operating mode (WDT enabled regardless of sleep; ((WDTCON0) and 0FFh), 0, b is ignored)
...
PSECT code, reloc = 2, abs ; vamos dar inicio al programa
 ...
 configuro:
    ...
    ; TRISF -> controla los pines del puerto F, puede ser entrada(1) o salida(o); en este caso ponemos el ((PORTF) and 0FFh), 3, a en 0 -> salida

    bcf ANSELF, 3, b ; EL ANSLEF define el pin como analogico (1) o digital(0) en el pin ((PORTF) and 0FFh), 3, a

    bsf LATF, 3, b ; BIT SET FILE -> pone un bit en 1 , en este caso el ((PORTF) and 0FFh), 3, a estara en 1
    ; LATF -> como es salida, define el valor que sale por el pin, al estar en 1 el ((PORTF) and 0FFh), 3, a-> va a salir 5v o 3.3 V
 ...
 inicio:
    ...
    btg LATF, 3, b ; BIT TOGGLE -> cambia el bit, osea si estaba en 1 lo pasa a 0 y asi, en este caso para el ((PORTF) and 0FFh), 3, a
